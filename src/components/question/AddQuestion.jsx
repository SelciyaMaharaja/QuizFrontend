...
const AddQuestion = () => {
	...
	const handleAddChoice = () => {
		if(choices.length <= 1 && choices[0] == "") {  alert("Please Add a Choice."); return; }
		const newChoice = ""
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (!validateInputs(result)) { alert("Please Enter All Required Details."); return; }
			...
			alert("Problem in Question Creation.");
			console.error(error)
		}
	}

	const validateInputs = (result) => {
		if(result.question == "" || result.questionType == "" || result.subject == "" || result.choices.length <= 0 || result.choices[0] == "" || result.correctAnswers.length <= 0 ||  result.correctAnswers[0] == "" ){return false;}
		return true
	}
